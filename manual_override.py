...
def test_combo_counter(timeout=2):
    ...
    start_time = time.time()  # <<-- Always start timing now!
...
def manual_menu():
    ...
    while True:
        ...
        print("12. Simulate Nighttime (Close Coop + Motion Demo + Count Summary)")
        print("13. Spray Water (Manual Test)")  # <<-- NEW: spray test
        ...
        if choice == "1":
            ...
            log_event("Daily Count Summary", f"Total chickens: {combo_count}")
            combo_count = 0
            print("[RESET] Combo counter reset for new day.")

            print("[SIM] Starting Motion Detection Demo. Press Ctrl+C to stop.")
            try:
                motion_handler()
            ...
            print("[SPRAYER] Activating water spray...")
            spray_water(times=5)  # <<-- Calls the sprayer
            log_event("Manual Spray", "Sprayer activated")
